...
async def _make_complete_assessment(db: AsyncSession) -> str:
    ...
    assessment = Assessment(
        ...
        overall_score=55.0,
        risk_tier="medium",
        dimension_scores={dim: dict(dim_score) for dim in [
            "model_inventory", "human_oversight", "bias_fairness", "data_governance",
            "incident_response", "monitoring_drift", "regulatory_compliance", "third_party_risk",
        ]},
...
@pytest.mark.asyncio
async def test_get_scores_complete_assessment(
...
async def test_get_scores_pending_assessment(client: AsyncClient) -> None:
    ...
    org = await client.post(
        "/api/v1/organizations", json={"name": "PendOrg", "slug": "pend-score-org"}
    )
    a = await client.post(
...
@pytest.mark.asyncio
async def test_get_scores_complete_but_no_scores_stored(
    client: AsyncClient, db: AsyncSession
) -> None:
